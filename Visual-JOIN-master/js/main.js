...
function JoinsCtrl($scope) {
  ...
  $scope.enjoys = [
    { user_id: 3, interest: 'Stars' },
    { user_id: 1, interest: 'Climbing' },
    { user_id: 1, interest: 'Code' },
    { user_id: 6, interest: 'Rugby' },
    { user_id: 4, interest: 'Apples' }
  ];
  ...
  $scope.sql = {
    ...
    inner: {
      ...
      desc: "INNER JOIN or just JOIN retrieves all users and enjoys that match each other ( where the id field in users matches a user_id in the enjoys table and vice versa )"
    },
    left: {
      query: "SELECT users.name, enjoys.interest FROM users LEFT JOIN enjoys ON users.id = enjoys.user_id;",
      desc: "LEFT JOIN retrieves all users and its enjoys if there is any else sets NULL in the interest field"
    },
    right: {
      query: "SELECT users.name, enjoys.interest FROM users RIGHT JOIN enjoys ON users.id = enjoys.user_id;",
      desc: "RIGHT JOIN is interest LEFT JOIN but retrieves all enjoys with all matching users or NULL if it doesn't have any matching user"
    },
    outer: {
      ...
      desc: "OUTER JOIN or OUTER LEFT and RIGHT with UNION (MySQL don't support FULL OUTER JOIN) retrieves all users and enjoys and matches them and sets NULL on any interest without any match on user and same thing with user with no matching interest"
    }
  };
